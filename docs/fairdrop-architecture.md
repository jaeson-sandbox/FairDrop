...
Directory staging walks with native `Lstat` and reparse checks, opens each directory only after inspection, and compares the opened handle's `Stat` identity with the inspected object before its first bounded positive `ReadDir(n)`. It then revalidates the path's non-link identity and rejects any opened child matching an active ancestor, preventing same-target link substitution and mount cycles with only the depth stack. It sums only non-negative regular-file sizes with checked `int64` arithmetic, retains only the reader stack plus one fixed batch, and closes every reader without retaining an entry index. The returned source path preserves the caller's spelling; trailing separators are trimmed only for metadata lookup beyond a volume/share root, and dot components derive their display name from already-inspected metadata rather than a cleaned path.
