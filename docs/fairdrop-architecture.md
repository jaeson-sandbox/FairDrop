...
Story 3.7 resolves selection ancestors at `App.StageTransfer`, before the coordinator and source inspection. This admits macOS selections beneath system aliases such as `/tmp` and `/var` without changing the traversal's no-follow rules. The final component remains unresolved, including when followed by a separator; explicit final dot traversal and Windows namespace spellings retain source validation. Failed ancestor resolution falls through to the existing source classification. Paths returned by the source preserve the spelling it receives after this boundary operation.
