...
- `ServerPort.Stop` is idempotent and force-closing, and bounded: it returns once the listener, active connection, handlers, payload workers, and server event producers have ended and its event channel is closed for good — or once its documented 10-second teardown bound elapses. A cleanup diagnostic means teardown finished; a structurally marked unquiescent failure means live work remains. The latter names the outstanding accept loop, handler, and/or connection, is replayed without creating new wait workers, and fences Start until persistent completion channels prove all three ended. Reuse recovers only after that proof. The server never holds its mutex across a wait.
...
- `NetworkPort.StartBeacon` requires a prior successful `GetLocalIP`: it advertises the address that call selected, and there is no address to advertise without one. A `StartBeacon` reached without that precondition is refused with `beacon_warning`. `StartBeacon` returns only after registration is active; on failure it cleans every partial registration before returning. Stop detaches ownership under lock and invokes `Shutdown` outside the selection/state locks. Concurrent stops join the same outstanding result; `GetLocalIP` remains responsive, while StartBeacon refuses until cleanup completes. A partial handle returned by failed or cancelled Start follows the same outstanding-cleanup rule. `StopBeacon` takes no context, so the coordinator supplies its own bound.
- Adapter Stop methods are safe before Start, after failed Start, and when repeated.
- Every coordinator wait is bounded: its adapter-call bound is 15 seconds, its drainer bound is 10 seconds, and its outer lease is 45 seconds. One outstanding call per adapter is retained across a timeout and joined by later attempts rather than relaunched. Stage checks those calls atomically with state/lease admission and returns `busy` until they finish. A server's structurally marked inner unquiescent result propagates; an ordinary returned cleanup diagnostic remains diagnostic-only. `Cancel` and `Shutdown` honour their caller contexts while joining another teardown. The healthy path still returns nil only after completion.
...
What Stories 3.4/3.8 add is the other case, because the prior paragraph did not say what happens when an adapter does *not* return. Bounds report failure and release the operation lease, but the in-flight adapter call remains owned and Stage remains `busy` until it returns; retries join rather than accumulate goroutines. The server's 10-second bound marks unproven quiescence structurally and retains persistent completion channels, so its later Start refuses until the accept loop, handlers, and connections truly end. The coordinator's 15-second bound therefore receives and propagates the more specific inner result instead of racing it. An ordinary cleanup diagnostic has no marker and remains non-fatal after proven quiescence.
...
| complete | required; see below when a port omits it | absent |
| error | optional | required `PublicError` |
| reset | absent | absent |

Every snapshot the coordinator publishes is enforced at that boundary rather than trusted (D-039): an unknown total is fixed at `totalBytes=0, percent=0`, a known-empty payload at `bytesSent=0, percent=0`, `bytesSent` is clamped to `totalBytes`, negatives become zero, and NaN or infinity are clamped into range. Enforced rather than refused because the frontend validator drops an incoherent event whole, and a dropped progress event is a meter that stops with nothing on screen to explain it.

A `ServerComplete` that carries no snapshot is a port defect, not a transfer failure: the bytes did arrive. The coordinator publishes `transfer-complete` with the unknown-total zero snapshot (`totalKnown=false`, `totalBytes=0`, `percent=0`, `bytesSent=0`) rather than downgrading a success the receiver actually got, and publishes no separate final-progress event for it. Reporting `transfer_failed` instead would lie about a completed transfer; omitting `progress` would break the required-payload rule above for every consumer. The frontend accepts that shape and ends the session, which is why a refused snapshot on a terminal event is still terminal (D-035).
...
- Preserve ordinary spaces and Unicode subject to the portable archive-segment refusals below. Support long Windows and UNC paths wherever native Go APIs permit; failures are typed and covered on capable native runners.
...
- File copy, ZIP-entry copy and archive drain fail on the 101st consecutive `(0,nil)` read with `transfer_failed` wrapping `io.ErrNoProgress`; any progress resets the count. This is a stalled-reader guard, not a deadline on a blocking native read.
