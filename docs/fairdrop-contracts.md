...
`SourcePort.Inspect` preserves `absolutePath` byte-for-byte in `StagedItem.Path`. For a directory it validates every syntactic ancestor and nested entry with cancellation-aware `Lstat` plus native reparse detection, then opens each directory and requires `os.SameFile` between the inspected metadata and the opened handle's `Stat` result before enumeration. After open it re-`Lstat`s and rechecks native reparse status and identity, so a same-target link substitution cannot pass the handle comparison. Every opened child is also compared with the active ancestor identities; a cycle is refused without retaining a global visited index. Enumeration uses a fixed positive `ReadDir(n)` batch and depth-first reader stack, sums only non-negative regular-file sizes with checked `int64` addition, closes every reader, and retains no entry index. A link-like, cyclic, or special entry is `path_unsupported`; a mismatched opened identity is `source_changed`; disappearance remains `path_not_found`; arithmetic faults are `transfer_failed`. Trailing separators may be removed only from the private metadata lookup path beyond a volume/share root, never from the returned path and never through destructive path cleaning. Dot-component display names come from the already-inspected metadata stack rather than path cleaning.
