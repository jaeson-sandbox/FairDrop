...
// OpenChildMetadata describes one child without opening its contents. Linux
// pins an O_PATH descriptor; Darwin takes a parent-relative no-follow snapshot.
func (n *posixNode) OpenChildMetadata(name string) (metadataHandle, error) {
	return openNativeMetadata(posixLocator{parent: n, name: name})
}
...
// OpenChildContent is the one file-content open in the package, and the only
