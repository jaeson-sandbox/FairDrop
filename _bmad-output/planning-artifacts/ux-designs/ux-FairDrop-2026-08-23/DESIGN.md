...
**Amended 2026-09-08** on the "Focus and outcomes" row: the outline half of that pairing is scoped to the two keyboard-Tab-reachable controls, `.fd-button` and `.fd-url`. The routed landing targets -- state headings, the cancel summary, and the Done/Error panel -- take focus by script only, sit outside the Tab order, and paint no outline at all. WCAG 2.4.7's visible-focus requirement does not reach a component that is not keyboard-operable, and a shared rule instead made every scripted focus move paint the ring regardless of input device: a transfer that completes on its own has no preceding interaction for `:focus-visible` to inherit a modality from, so the Done panel read as stuck "selected" long after the transfer finished -- reported from the running app. Each landing target's own glyph and heading text still carries the landing without a ring. See `frontend/src/style.css` and `frontend/src/ui/styles.test.ts`.
