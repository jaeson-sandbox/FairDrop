...
- **Review round 2 (2026-08-30, independent re-verification -- tests only, no production change):** Thirty-six mutations were applied to the six production modules and reverted; thirty-three failed a test named for the guarantee they broke, and the three survivors were closed here. **The session-less guard was entirely untested.** Replacing `if (!hasSession(state)) return state` with a condition that never fires left all 161 tests green, yet every lifecycle event arriving at Idle or Pending then dereferenced a `session` that does not exist and threw out of the reducer. Nothing in the suite had ever dispatched a lifecycle event at a state that owns no session -- precisely the states a forged event finds before the first Stage and after a reset, which is the threat Story 1.7 handed to this story. The new test drives all five event names, in three valid payload shapes, at a fresh Idle, a Pending, and a reset-retained Idle. **`fixedCopy` was undistinguishable**, because every call site is already fed registry copy by the validators: returning the argument unchanged broke nothing. The reducer is exported and its actions carry a caller-built `PublicError`, so the rewrite is now pinned where a caller supplies the copy -- `stage-failed` and `active-cancel-failed` with a known code carrying a path and token -- which is the acceptance criterion's own wording rather than the parser's. **Two Cancels in one tick both fired.** `cancelPending` cannot deduplicate them, since both activations read the same pre-dispatch state; only `activeCancelRef` can, and no test proved it did. All three were verification gaps, not defects: no production line changed in this round, and each mutation was re-run afterwards to confirm the new test names it. **One artifact regression was repaired outside the story's own files:** the 2026-08-29 `compile-epic-context` run overwrote `epic-1-context.md` with a shorter compile that dropped the copy-registry rule, the naming and event conventions, the entire receiver-help paragraph Story 1.9 renders from, "every focus target proven to exist before focusing", and Epic 1's own native exit check. The richer prior text is restored with the two things that regeneration genuinely added folded in, plus the forged-event rule stated where the next story will read it.
