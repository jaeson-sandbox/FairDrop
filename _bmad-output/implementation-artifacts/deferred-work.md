...
- source_spec: `spec-6-1-replace-the-placeholder-app-icon.md`
  id: D-128
  summary: The shipped exe's Properties -> Details pane reads back empty ProductName, FileDescription and CompanyName, because build/windows/info.json declares its string table under language block "0000" rather than "0409".
  owner: accepted
  evidence: Found on 2026-09-19 while verifying Story 6.1's manual-check row, which asks a human to look at the exe's Properties -> Details pane. The icon half passed -- the extracted 32x32 icon measures 0.57 central saturation, the copper mark -- but `(Get-Item fairdrop.exe).VersionInfo` returns empty strings for `ProductName`, `FileDescription`, `CompanyName` and `FileVersion`. **Not a regression from this story:** `build/windows/info.json` is byte-identical to the Wails 2.15.0 scaffold default and has not been touched since `0ec79ac` (2026-08-22), so every release including v1.0.0 shipped this way. The cause is the `"0000"` key in `info.json`'s `info` map: that is a language-neutral block, and the Win32 version API resolves `VersionInfo` through a specific language/codepage -- US English is `"040904b0"`, commonly written `"0409"`. This is also a verification gap rather than only a cosmetic defect: `release_identity_test.go` pins ProductName, ProductVersion and CompanyName as they appear **in info.json's template**, and passes, while nothing pins what those fields become in the built artifact -- the template is verified, the product is not. Recorded rather than fixed because Story 6.1's Code Map marks `info.json` read-only and its Never clause forbids widening scope; a one-character-class change to the language key is cheap but changes what ships, so under the governance rule it needs its own story. Whoever takes it should pin the built exe's resources, not just the template, or the same gap survives the fix.
