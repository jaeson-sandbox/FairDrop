...
- source_spec: `spec-6-1-replace-the-placeholder-app-icon.md`
  ...
  owner: accepted
  evidence: Raised by Story 6.1's verification-gap review layer on 2026-09-19 and confirmed against the repo. All three of `appicon_test.go`'s assertions open `build/appicon.png` or `build/windows/icon.ico`. Nothing reads `build/bin/fairdrop.exe`: grepping `build/bin` across `*.go` returns a single hit, `single_instance_darwin_test.go:195`, which is macOS-only and env-gated on `FAIRDROP_NATIVE_APP_SMOKE` plus `GITHUB_ACTIONS`. So if the resource-embedding path regressed -- `compileResources`' `.syso` step, or an `.ico` the resource compiler cannot parse -- the exe would fall back to the shell default icon and all three assertions would still pass. This is not hypothetical at this layer: [[D-128]] records the sibling resource block, the version strings in the very same Properties -> Details pane, reading back empty for every release through v1.0.0 while `release_identity_test.go` pinned those fields in the template and passed throughout. The icon is currently pinned at exactly the layer that already missed a real shipped defect. The repo has the pattern: an env-gated test shaped like `TestDarwinBuiltAppSurvivesUnusableLock` (`single_instance_darwin_test.go:155`), driven by its own `verify.yml` step after `wails build`, extracting RT_GROUP_ICON/RT_ICON and running the existing saturation assertion against it. Deferred rather than fixed in Story 6.1 because that story's Never clause forbids touching `verify.yml`, and one step should cover this and [[D-128]] together rather than being built twice.

- source_spec: `spec-6-1-replace-the-placeholder-app-icon.md`
  id: D-130
  summary: verify.yml runs `wails build` before `go test`, so an icon.ico deleted from the repo and never re-committed would be regenerated on the runner and satisfy the icon assertions against a file absent from git.
  owner: accepted
  evidence: Raised by Story 6.1's verification-gap review layer on 2026-09-19. `wails build` regenerates `build/windows/icon.ico` whenever it is absent (`packager.go:204`), and `verify.yml` runs that step before `go test`, so on CI the icon assertions can be satisfied by a file the runner just produced rather than by the committed artifact. The workflow's existing post-build drift check is scoped to `frontend/wailsjs` and `frontend/dist/.gitkeep` and does not cover `build/`. The blast radius is narrow -- this masks an `.ico` missing from git, not wrong artwork, and locally the tests read the committed file -- but it means CI is not checking what it appears to check. Deferred because the fix extends the post-build drift check in `verify.yml`, which Story 6.1's Never clause forbids touching, and it belongs with [[D-129]]'s CI step rather than being wired separately.
