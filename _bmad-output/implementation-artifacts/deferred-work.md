...
- source_spec: `spec-1-10-meet-the-accessibility-and-recovery-contract.md`
  ...
  owner: discharged
  resolution: Story 3.12; the half a runner can produce now exists and the half it cannot is recorded as owed rather than implied. `frontend/browser/accessibility.test.tsx` renders Staged in real Chromium with forced colors emulated and asserts that `.fd-qr-panel` and `.fd-qr` are the only elements computing `forced-color-adjust: none` -- proved by mutation from both directions: a second selector taking the exemption inside the media block fails the check, and disabling the emulation itself fails it too, so the suite cannot pass by not emulating. The panel is captured to `frontend/browser/captures/qr-panel-forced-colors.capture.png`, named "capture" rather than "scan" because that is what it is: it shows the substrate still painting light-on-dark with its quiet zone intact, and proves nothing about a camera. The camera reading is now release-evidence row 14, optional and unverified, which is where an observation nobody owes belongs under `docs/release-policy.md`. `DESIGN.md`'s gate is therefore still unmet, and now says so in the one file a person reads before a release rather than in a backlog.
...
- source_spec: `spec-1-10-meet-the-accessibility-and-recovery-contract.md`
  ...
  owner: discharged
  resolution: Story 3.12; a second Vitest project drives real Chromium through the Playwright provider and measures what jsdom cannot evaluate -- 320px reflow, 200% text, the 44px activation floor, and forced colors -- failing on a measured value that names the element. The text assertions in `styles.test.ts` stay: they run everywhere in milliseconds and catch a token edit before the browser launches, and the rendered pass is a second opinion rather than a replacement. It found a real failure on its first honest run: `aria-disabled` dimmed the quiet Cancel button with `opacity: 0.7`, compositing muted-on-elevated from its published 4.504478335:1 to 2.64:1 light and 4.01:1 dark. The screen-reader half of this id does not close and never could -- rows 10 and 11 of the release evidence stay optional and unverified, because what NVDA and VoiceOver actually say is an observation, not an assertion.
...
- source_spec: `spec-1-10-meet-the-accessibility-and-recovery-contract.md`
  ...
  resolution: Story 3.12; both missing layers ran against Story 1.10's change and every finding was revalidated against current code rather than against the diff, since two epics of other work had moved underneath it. It earned its cost. Blind Hunter found four live defects that two epics of review had walked past -- the `opacity: 0.7` contrast failure above; a clipboard rejection discarded by the staged view, which left `clipboard_failed` with a registry message and no path to a user; a copy confirmation that outlived the attempt it described; and a copy resolving behind a cancel, replacing "Canceling…" with an answer to a question the user had stopped asking. All four are fixed here with named tests. It also found `EXPERIENCE.md` describing the assistive-speech throttle per-mode while the code has always applied both thresholds in both modes -- the owner chose to correct the spine, and the two are pinned to each other now. Two findings were checked and needed no change: the transition effect's missing dependency array is deliberate (it observes commits, not state, and returns early when the state is unchanged), and D-059's dead terminal control was already fixed earlier in this story.
...
- source_spec: `spec-3-11-close-the-residual-contract-and-copy-gaps.md`
  ...
  owner: discharged
  resolution: Story 4.1; a fifth code, `chooser_failed`, replaces the `transfer_failed` that `app.go`'s `chooseWith` used to return for a dialog that never opened. Its message reads "FairDrop couldn’t open the chooser. Try again, or drop the item on the zone above." -- the first wording said "drag the item onto the window", which the review caught as false: `OnFileDrop` is registered drop-target-gated and `--wails-drop-target: drop` sits only on `.fd-drop-zone`, so a drag anywhere else is ignored, and EXPERIENCE.md's own Recovery cell for the same row already said to use the drop zone. The code moves through eight places together under the existing cross-language pin, not the five the spec first named: EXPERIENCE.md, docs/fairdrop-contracts.md, internal/transfer/errors.go, internal/transfer/errors_test.go, frontend/src/transfer/errors.ts, frontend/src/transfer/errors.test.ts, frontend/src/ui/copy.test.ts and main_test.go's own registryEntries literal, which drives the pin and would otherwise have stopped checking a fifth code silently. No path in the message (AD-9).
...
- source_spec: `spec-3-12-capture-the-accessibility-evidence-a-runner-can-produce.md`
  id: D-114
  summary: A successful copy renames the Copy button to "Copied" for the rest of the session, so the control loses the name that says what it does.
  owner: discharged
  resolution: Story 4.1; the label reverts to copy.direct_link.action the moment focus leaves the control (a blur handler), which EXPERIENCE.md now sanctions as the trigger for this control specifically -- not a timer, and driven only by the sender's own focus move, so a keyboard trap is never introduced. By the time the sender returns to the control, its name already describes what it does again.
  evidence: Found by the Blind Hunter layer re-run (D-109), beside three defects in the same control that were fixed in Story 3.12. `StagedView` swaps the button's whole label to `copy.copy.confirmation` on success and never swaps it back -- no timer, because EXPERIENCE.md bans frontend lifecycle timers, and nothing else in the staged session changes the flag. The visible text and the accessible name are the same string, so WCAG 2.5.3 is satisfied either way; what is lost is 4.1.2's name. `review-accessibility.md` asks for "a semantic **Copy download link** button whose accessible name includes 'download link'", and after one copy the only control that reaches the capability URL is called "Copied". A screen-reader user arriving at it later -- or anyone returning to the window minutes after copying -- is told the outcome of an action they can no longer identify. Not fixed in 3.12 because every fix is a design decision the owner holds: swapping the label back needs a trigger EXPERIENCE.md does not sanction, keeping the accessible name while showing "Copied" breaks label-in-name, and moving the confirmation to a sibling element changes the shape of a control DESIGN.md lays out. `copy.copy.confirmation` is registered contract copy (EXPERIENCE.md line 96); where it renders is not, which is exactly the gap. Routed to Story 4.1 because it rebuilds the selection controls and is the next story that touches this surface with the owner's attention on it.

- source_spec: `epic-3-retro-2026-09-14.md`
  id: D-115
  summary: A transfer whose bytes all reached the receiver can still report cancelled, and the fix the owner approved rests on a premise that turned out to be false.
  owner: accepted
  evidence: Found as B6 in the Epic 3 retrospective and taken to the owner on 2026-09-15, who chose "deliver the recorded outcome" over keeping the suppression. Implementing it showed the framing was wrong, so the decision needs remaking on correct facts rather than executing. `internal/server/lifecycle.go`'s `trackConnection` delivers a recorded terminal event only when `!stopping && r.ctx.Err() == nil`, and the retrospective argued that a recorded terminal means the transfer genuinely finished, so suppressing it tells a sender "cancelled" about a transfer the receiver has in full. The first half of that is false. `finalizeAfterResponse` records `connection.terminal` when the *handler* stops writing into the buffer, not when net/http has flushed the bytes to the socket -- which is the whole distinction Story 3.7 built the finalization mechanism to make (D-110: natural completion waits for the actual final body and framing writes observed at connection closure). Removing the guard fails `TestNaturalCompletionWaitsForHTTPFinalization` and `TestHeaderOnlyFinalizationWaitsAndRetainsFailureCodes`, both of which gate the final write, call `Stop`, and require that no terminal outcome is published -- correctly, because those bytes never left. A second rule sits underneath, stated in `handler.go`: during a teardown the coordinator owns the outcome and the server stays silent, so even a failure event is unwanted there. `finalizingConn` carries only `writeErr` and `terminal`, and neither separates "flushed, then a stop landed" from "force-closed mid-write", so there is no signal available to deliver one case and suppress the other. Closing this properly means either recording that the final flush completed and letting only that case through, or renegotiating the silent-teardown rule -- both are contract changes that belong to a story with the owner's attention, not to a fix applied on a premise that did not survive contact with the code. Marked accepted rather than routed because nothing is known-broken: the current behaviour is conservative and the cost is a narrow, rare mislabel.

- source_spec: `epic-3-retro-2026-09-14.md`
  id: D-116
  summary: publish still spawns a goroutine and a timer per event; the cap that fixes it made an existing test-harness assumption break on macOS under the race detector.
  owner: accepted
  evidence: B9 in the Epic 3 retrospective. `Coordinator.publish` routes every event through `callBounded`, and `forwardProgress` calls it once per accepted snapshot, so an observer that stops returning leaves one abandoned goroutine per snapshot. The stronger reason to cap it is the port contract: `Observer.Publish` is declared "a synchronous FIFO handoff ... an implementation must deliver in call order", and a goroutine per event lets an abandoned call be entered concurrently with the next one. A cap holding one outstanding call was written, proved by three mutations (no cap, a slot that never clears, terminal events exempted) and green on every local gate including 30 race iterations -- then failed `TestATeardownThatLosesTwoResourcesReportsBoth` on macOS under `-race` in CI with "Cancel never returned". The mechanism is the test harness, not the cap's logic: `fakeTimer.fire()` runs the **most recently armed** callback, and that test fires two bounds in sequence on the stated assumption that the one it wants is the only one pending. Any publish that arms its own bound in the same window is fired instead, the intended bound never fires, and the teardown waits out its full bound. The cap adds work inside the bounded call's closure, which widens that window enough to lose the race on a slow runner. Reverted rather than shipped: the change could not be verified where it fails, and a fix that distorts production timing to satisfy a harness would be the wrong direction. Closing it properly means giving the timer stub a way to fire a *named* bound rather than the newest one, so a test says which bound it means -- that is a harness change several tests depend on, and belongs to a story that can re-run the macOS gate rather than to a release branch.

- source_spec: `epic-4-coherence-and-release`
  id: D-117
  summary: Two tests assert on wall-clock sleeps and fail on a loaded macOS runner, one of them intermittently reporting a success where a deadline should have expired.
  owner: accepted
  evidence: Found while getting this branch's gate green, on changes that cannot have caused either. `internal/stream/payload_test.go`'s `TestPrepareDistinguishesDeadlineExpiryDuringOpenFromCancellation` builds a 20ms context deadline and a 60ms `time.Sleep` inside the open seam, then requires Prepare to report `setup_failed` wrapping `context.DeadlineExceeded`. It failed on macOS with `error = nil` -- Prepare succeeded -- while the same commit passed on Windows and Linux, and this branch's only change to that package is comments (`git diff main HEAD -- internal/stream/` has no code lines, and `payload_test.go` is untouched). A sibling in `internal/transfer` has the same shape from the other direction: `TestATeardownThatLosesTwoResourcesReportsBoth` failed once on macOS under `-race` with "Cancel never returned", and its mechanism is `fakeTimer.fire()` running the most recently armed callback while the test assumes the bound it wants is the only one pending (see D-116). Both are the pattern this project's own notes name -- an assertion that is only sometimes true -- and both are invisible locally: 30 race iterations of the second passed on Windows, and the macOS runner in these runs took 513 seconds for `internal/stream` alone under `-race`, so it is slow enough to reorder anything built on sleeps. Closing it means driving these two through the seams their packages already have (the fake clock, the fake timer) rather than through wall-clock time, which is a test-harness change rather than a product one and wants a story that can re-run the macOS gate on demand. Accepted rather than routed because nothing is known-broken in the product: what is unreliable is the proof, not the behaviour it covers.

- source_spec: `spec-4-1-replace-the-two-browse-controls-with-one.md`
  id: D-118
  summary: `not_ready`'s public copy blames a second instance, but it is also the code every command that arrives before the window exists reports, including one that could not open a chooser.
  owner: accepted
  evidence: Raised by the Blind Hunter layer reviewing Story 4.1, which noticed that `chooseWith`'s nil-context branch still answers `not_ready` four lines above the branch D-113 just moved off it — and that `not_ready`'s registry message reads "FairDrop isn't ready to send. Another copy may already be running. Close this window and use that one", a sentence about a second instance that a missing runtime context is not. The observation is right; the suggested fix is not. `app_test.go`'s `TestDialogBeforeStartupIsRefusedRatherThanFatal` pins `not_ready` there on purpose, and its comment records why: D-048 and D-104 established that every command arriving before a window exists or before compose ran answers `not_ready`, consistently across StageTransfer, CancelTransfer and both dialogs. Re-coding one of those branches to `chooser_failed` because this story happened to be looking at it would trade a coherent convention for a locally better sentence, and would leave the other pre-window branches still saying the thing the reviewer objects to. What is actually wrong is the copy: one code covers two situations a user would describe differently — an instance conflict, and a window that does not exist yet — and only the first is in the message. Fixing that properly means either splitting the code or rewording it so it covers both without naming a cause it cannot know, and either is new public copy, which is the Ask First boundary of every story here. Accepted rather than routed: the branch is documented unreachable in a composed binary (Wails runs OnStartup before the webview can issue a command), so nothing a user meets today depends on it.

- source_spec: `spec-4-1-replace-the-two-browse-controls-with-one.md`
  id: D-119
  summary: EXPERIENCE.md's copy registry is the one contract table nothing pins, so a new user-facing string can ship with no row and no test noticing.
  owner: discharged
  resolution: Closed 2026-09-17, immediately after Epic 4, on the owner's instruction to make the scope call rather than hold the finding for a decision. `copy.test.ts` gains a third describe that reads the spine's Voice and Tone table and pins it both ways: every row must name a real leaf and quote its string exactly, and every leaf outside `label` and `unit` must have a row. That split is copy.ts's own documented provenance rule, now enforced instead of merely described. Six mutations, all killed by the named test that should see them: reword a row, reword the code, drop a row, point a row at a missing leaf, add approved copy with no row (the exact D-119 scenario), and break the table parser -- the last caught by a guard asserting the parser found more than thirty rows and one known id/value pair whole, so an empty parse cannot pass the other two vacuously. What stays human is whether a new `label` entry is approved copy that earns a row or a structural word the spine names in prose; nothing mechanical separates `Choose a file or folder` from `Wire bytes`, and the test comment says so rather than implying the hole is closed.
  evidence: Found reviewing Story 4.1, which shipped three new strings -- `copy.label.chooseFileOrFolder`, `copy.label.file` and `copy.label.folder` -- and whose registry rows were missing until they were added by reading, not by a failing test. Every neighbouring contract in this project is pinned mechanically: the cross-language *error* registry moves through eight places under `TestTheCrossLanguageErrorRegistryPinsEveryCodeAndMessage` and `TestPublicErrorOfExactRegistryCopy`, `copy.test.ts` pins `errorHeadings` against the code set, `announce.test.ts` takes one case per row of EXPERIENCE.md's Announcement-ownership table, and `main_test.go` pins the deferred ledger, the sprint-status keys and the mutation script's test names. The "Message and Tone" table in EXPERIENCE.md, which `copy.ts`'s own header calls "approved product copy", is read by nothing: `grep` for EXPERIENCE across the Go and TypeScript trees returns comments only. The gap is one-directional and that is what makes it narrow -- a string in the table with no `copy.ts` entry is a dead row, harmless; a string in `copy.ts` with no row is unapproved copy on the screen, which is the case this story nearly shipped. Closing it means a test that parses the table's id/value columns and requires every leaf of `copy` to appear, which needs a decision about which `copy.ts` leaves are registry copy and which are structural labels (`copy.label.windows`, `copy.label.macos` and the firewall heading are arguably the latter), plus a Markdown-table parser in a suite that has none. Accepted rather than routed because nothing on screen is wrong today -- the three rows are in place and the registry and `copy.ts` agree -- and because the decision about scope is the owner's, not a mechanical fix.

- source_spec: `spec-5-1-split-internal-transfer-coordinator-go.md`
  id: D-120
  summary: The diagnostic sink's cap is asserted against itself, so any positive `maxDiagnostics` passes the test identically.
  owner: discharged
  resolution: Story 5.1, fixed on the story's own branch as a separate test-only commit -- AGENTS.md's 2026-09-17 rule permits a test-only pin on a named branch without a story, so waiting for the story to merge was never required and the first draft of this entry said otherwise. `coordinator_outcomes_test.go:975` now asserts against the literal 32 with `:976`'s message following it; `:967`'s loop bound and `:988`'s seam count stay derived, because those two are meant to move with the constant. Two corrections to this entry's own first wording: the vacuity is not total -- at a cap of 0 the test fails, so the floor is pinned and only the magnitude was not -- and the account of what the test covers omitted its third assertion, that the Diagnose seam sees every diagnostic the cap discards.
  evidence: Found by mutation while verifying Story 5.1, not by the refactor itself -- the vacuity is pre-existing and predates the move. `internal/transfer/coordinator_outcomes_test.go`'s `TestTheDiagnosticSinkSaysWhenItStoppedRecording` drives `maxDiagnostics + 8` diagnostics through `recordDiagnostic` and then asserts `len(entries) != maxDiagnostics`. Both the input and the expectation are derived from the symbol under test, so changing the constant moves both sides together: a mutation from 32 to 8 left the whole suite green. This is vacuous-test pattern 1 in the project's own notes -- assert against a literal written at the assertion site, never against the symbol under test. What the test does genuinely pin is the overflow marker, which is its stated purpose and the reason it exists: a separate mutation making `record` a no-op was killed by this test and two others, and the marker's position and non-repetition are both asserted against `diagnosticOverflow` directly. So the gap is narrow -- only the numeric cap is unpinned, and the comment on the constant treats 32 as a bound rather than a contract ("a session produces a handful at most"). Not fixed inside Story 5.1 on purpose: the fix is one literal in a `_test.go` file, and that story's defining acceptance criterion is that no `_test.go` file appears in its diff at all. Breaking that bar to fix an unrelated pre-existing vacuity would have destroyed the one property that made a move-only refactor provable. AGENTS.md's 2026-09-17 governance rule explicitly allows a test-only pin on a named branch without a story, so this is closable immediately after 5.1 merges by replacing the assertion's `maxDiagnostics` with the literal 32 while leaving the loop bound derived.

- source_spec: `spec-5-1-split-internal-transfer-coordinator-go.md`
  id: D-121
  summary: `unportableNamesWarning` is executed by no test, so the Staged warning for Windows-unsafe filenames can carry the wrong code or copy and ship green.
  owner: discharged
  resolution: Story 5.1, fixed on the story's own branch as a separate test-only commit alongside D-120. A `coordinator_stage_test.go` case now stages an item with UnportableNames above zero and asserts the published Staged metadata carries WarnUnportableNames with the ErrNameWarning registry message, mirroring how TestStageCommitsWithAWarningWhenOnlyTheBeaconFails pins the beacon side.
  evidence: Found by the Verification Gap review layer of Story 5.1 and reproduced independently before acceptance. Two mutations of `internal/transfer/warnings.go` -- giving unportableNamesWarning the beacon warning code, and building it from the wrong registry entry -- both left all 8 packages green. No test in the repository stages an item with UnportableNames above zero: `helpers_test.go`'s testItem() leaves the field at its zero value, and the only two inspect overrides (`coordinator_stage_test.go:321` and `:714`) return errors, so the branch in Stage never executes. The frontend tests naming name_warning assert the TypeScript registry against hardcoded fixtures and cannot observe what Go emits. The consequence is not cosmetic: `frontend/src/transfer/validation.ts:66` rejects any warning code outside beacon_warning/name_warning and returns null for the whole Staged metadata record, so a wrong code here shows the sender nothing staged rather than a mislabelled warning. Pre-existing rather than introduced by the split -- the 194/194 verbatim check proves the move altered nothing -- but the split is what put the function in a two-declaration file whose coverage was claimed on a six-mutation sample that never touched it.

- source_spec: `spec-5-1-split-internal-transfer-coordinator-go.md`
  id: D-122
  summary: `releaseAcquired`'s switch over resource has no default, and the resource constants now live in a different file, so a third resource would compile and never be released.
  owner: accepted
  evidence: Raised by the Edge Case review layer of Story 5.1 and verified. `internal/transfer/coordinator.go`'s releaseAcquired walks live.acquired in reverse and switches on each entry with cases for resourceBeacon and resourceServer and no default. Story 5.1 moved the resource type and its const block to `session.go`, so the value set and its exhaustive switch now sit in different files with nothing but attention connecting them: adding a third constant compiles cleanly and the resource is silently never released, stranding a listener or a beacon. The switch always lacked a default; what the split changed is that a developer adding a resource no longer has the switch on screen. Not fixed inside Story 5.1 because releaseAcquired is named in that story's Never clause, which exists so a move-only refactor cannot quietly become a behaviour change. Accepted rather than routed: there are exactly two resources, both have cases, and adding a third is not planned work -- the cost of the gap today is zero, and a default arm reporting a coded failure is a five-line change whenever a third resource is actually proposed.

- source_spec: `spec-5-1-split-internal-transfer-coordinator-go.md`
  id: D-123
  summary: `capabilityURL` narrows its port to uint16 unchecked, and the only range guard now sits in a different file.
  owner: accepted
  evidence: Raised by the Edge Case review layer of Story 5.1 and verified. `internal/transfer/identity.go`'s capabilityURL does netip.AddrPortFrom(address, uint16(port)) with no validation; the range check that makes that safe is `coordinator.go:336`'s guard refusing a handle port below 1 or above 65535, applied before the URL is ever built. The precondition is real and currently enforced, but it and its only enforcement now live a file apart, so a second caller would wrap silently and produce a URL pointing at the wrong port. Accepted rather than fixed: capabilityURL has exactly one caller, the guard runs before it on every path, and adding either a redundant check or a precondition comment would deviate from the story's Always clause requiring moved declarations to be byte-identical apart from their new file. Worth a doc comment the next time this function is edited for any other reason.

- source_spec: `spec-5-1-split-internal-transfer-coordinator-go.md`
  id: D-124
  summary: `scripts/verify-native-mutations.sh` carries a hard-coded backup list that does not include the four files Story 5.1 created.
  owner: discharged
  resolution: Story 5.1, fixed on the story's own branch as a test-infrastructure commit alongside D-120 and D-121. The four new files are added to the script's files list so anything it mutates in them is restored.
  evidence: Raised by the Blind Hunter review layer of Story 5.1. `scripts/verify-native-mutations.sh:8` declares the array naming every file the script backs up before mutating and restores afterwards, including `internal/transfer/coordinator.go` and `internal/transfer/bounded.go`. Story 5.1 created four new files in the same package and none was added. Nothing breaks today -- verified that no mutation the script performs targets moved code; its internal/transfer mutations hit cleanupPending and AdapterCleanupBound, both of which stayed in coordinator.go -- so this is a latent gap rather than a live one, which is why neither the script passing nor the story's gates caught it.

- source_spec: `spec-5-1-split-internal-transfer-coordinator-go.md`
  id: D-125
  summary: `randomHex`'s nil-entropy fallback is unreachable, and no mutation can catch it.
  owner: accepted
  evidence: Raised by the Blind Hunter review layer of Story 5.1. `internal/transfer/identity.go`'s randomHex opens by taking the coordinator's entropy and falling back to rand.Reader when it is nil, but NewCoordinator already defaults entropy to rand.Reader and is the only construction site that matters -- the sole Coordinator literal at `coordinator.go:196` is internal and populated. A mutation deleting the fallback survives, and a mutation setting the source to nil is not a valid mutant at all: it fails the build rather than the test, which proves nothing about coverage. Accepted rather than removed: it is defensive code costing two lines, the same shape as ready()'s documented residual path for a Coordinator assembled some other way (D-029), and deleting it would be a behaviour change inside a story whose whole premise is that nothing changes. Recorded so a future reader does not mistake its survival under mutation for a coverage gap.

- source_spec: `epic-5-retro-2026-09-17.md`
  id: D-126
  summary: Sixteen OS-specific files sit in the root package where Wails only ever scaffolded two, and an internal/platform package would take the root from 25 Go files to 9.
  owner: accepted
  evidence: Raised by the owner on 2026-09-18 while reviewing how the repository presents itself publicly, and investigated rather than assumed. The textbook Go layout is **not** reachable here, for two constraints both verified: `main.go` carries `//go:embed all:frontend/dist`, and Go's embed cannot reference a parent directory, so `main.go` must stay at the root with `frontend/` beneath it -- `cmd/fairdrop/main.go` would require moving the whole frontend under `cmd/`. And moving the bound `App` struct out of `package main` changes Wails' generated binding path from `wailsjs/go/main/App` to `wailsjs/go/app/App`, which breaks four frontend imports (`App.tsx`, `App.focus.test.tsx`, `App.test.tsx`, `useTransfer.test.tsx`) and every binding-drift check. What **is** cleanly movable is the sixteen platform files -- `instance_lock*`, `single_instance*`, `theme_*`, `fatal_dialog_*` -- which carry no binding surface and no embed: `internal/platform/` would take the root from 25 Go files to 9, which is the whole of the visual problem. Deliberately excluded: the meta-tests (`release_identity_test.go`, `verify_workflow_test.go`, `main_test.go`) read `wails.json`, the workflow files and the ledger by path relative to their package, so moving them means `../../` prefixes throughout and makes them worse rather than better. Accepted rather than routed: `main.go` and `app.go` at the root is exactly what `wails init` scaffolds, so the layout reads as idiomatic to anyone who knows the framework, and nothing is harmed by the current arrangement. If it is ever taken, it is a move-only refactor and should carry Story 5.1's acceptance bar -- zero `_test.go` edits, byte-identical behaviour, and `git status` showing no binding drift.

- source_spec: `epic-5-retro-2026-09-17.md`
  id: D-127
  summary: Two commits carry a university email address that cannot be removed without rewriting every commit SHA the artifact trail cites.
  owner: accepted
  evidence: Found by the pre-publication security audit on 2026-09-18. Of 269 commits, 267 are authored `mail2jaeson@gmail.com` and two -- `20c211d` (the initial commit) and `8b6ce6f` (a pull-request merge) -- are authored `xac7gz@virginia.edu`. Both were made through GitHub's web interface, which used the account's default address at the time rather than the local git config. This is not a secret: the same person is identified by name and gmail address in the other 267 commits, so the disclosure is institutional affiliation rather than identity. Removing it requires rewriting author metadata from the initial commit onward, which changes every commit SHA in the repository. That cost is unusually high here: the published `v0.1.0` through `v1.0.0` tags would all be invalidated, and the evidence files, retrospectives and spec Change Logs cite specific SHAs throughout as provenance -- rewriting would turn hundreds of citations into dangling references and destroy the traceability the artifact trail exists for. Accepted on the owner's judgement that the exposure does not justify that. Recorded so a future reader does not mistake the inconsistency for an unnoticed accident. Prevention rather than repair: GitHub's "Keep my email address private" setting stops web-UI commits from using a real address again.

- source_spec: `spec-6-1-replace-the-placeholder-app-icon.md`
  id: D-128
  summary: WITHDRAWN as a defect -- the shipped exe's version resources are correct and complete; the emptiness reported twice in earlier drafts was an artifact of reading them with PowerShell, which cannot see a language-neutral string table.
  owner: discharged
  evidence: Raised on 2026-09-19 while verifying Story 6.1's manual-check row, and wrong twice before being settled by direct inspection. Kept rather than deleted because the reasoning is the useful part. **What is actually true,** established by calling Win32 `VerQueryValue` through `version.dll` -- the API Explorer itself uses -- against `build/bin/fairdrop.exe`: the file declares `VarFileInfo\Translation` as lang 0x0000 / codepage 0x04b0, and at `\StringFileInfo\000004b0\` every field is present and correct: CompanyName `FairDrop`, FileDescription `FairDrop`, ProductName `FairDrop`, ProductVersion `1.0.0`, LegalCopyright `Copyright (c) 2026 Jaeson Martin`, Comments `Ephemeral local P2P file transfer`. FIXEDFILEINFO carries 1.0.0.0. The templating worked; nothing is blank in Explorer. **Why two earlier drafts said otherwise:** both measured with `(Get-Item fairdrop.exe).VersionInfo`, which is .NET's `FileVersionInfo`. That class does not honour a langID-0 neutral table -- it probes `040904b0`/`040904e4` and returns empty strings rather than failing -- so a correct file reads back blank. The first draft then asserted info.json's `"0000"` key as the cause and cited a Story 6.1 Code Map entry marking info.json read-only; the Code Map never mentions info.json, so that citation was invented. The second draft corrected the citation but still carried the defect as real. **The residue is small and genuine:** `info.json` declares no `FileVersion` **string** (only `fixed.file_version`, which lands in FIXEDFILEINFO), so tools reading that specific string see nothing; and the neutral langID means .NET-based tooling cannot read any of these values, which is a portability wart rather than a defect. Both are picked up by Story 6.2 along with [[D-129]], not because anything is broken but because they are free to fix while that story is already reading the built exe. **The lesson worth keeping:** this entry asserted a shipped defect twice from a single tool's output without inspecting the artifact, which is the same error [[D-129]] exists to prevent -- verify the product, and verify it with something that can actually read it.

- source_spec: `spec-6-1-replace-the-placeholder-app-icon.md`
  id: D-129
  summary: Nothing verifies that the built exe actually embeds the committed icon; every automated assertion reads the input assets under build/, never the product under build/bin/.
  owner: discharged
  evidence: Raised by Story 6.1's verification-gap review layer on 2026-09-19 and confirmed against the repo. All eight of `appicon_test.go`'s tests open `build/appicon.png`, `build/windows/icon.ico` or `build/appicon-source.jpg`. Nothing reads `build/bin/fairdrop.exe`: grepping `build/bin` across `*.go` returns three hits, of which two (`release_identity_test.go:109` and `:112`) are string literals inside error messages, leaving `single_instance_darwin_test.go:195` as the only code that opens a built binary -- and that is macOS-only and env-gated on `FAIRDROP_NATIVE_APP_SMOKE` plus `GITHUB_ACTIONS`. (An earlier draft of this entry said "a single hit"; the conclusion survives, the measurement did not.) So if the resource-embedding path regressed -- `compileResources`' `.syso` step, or an `.ico` the resource compiler cannot parse -- the exe would fall back to the shell default icon and every assertion would still pass. **Why this is worth doing rather than assuming:** an earlier draft argued from [[D-128]] as precedent, claiming the sibling version-string resources had shipped empty for every release; D-128 is now withdrawn, because that claim came from reading the exe with PowerShell's .NET `FileVersionInfo`, which cannot see a language-neutral string table. The real lesson points the same way but more sharply: the only two times this repo inspected the built artifact's resources, it drew a wrong conclusion once from the wrong tool and had no automated check either time. The repo has the pattern to fix it -- an env-gated test shaped like `TestDarwinBuiltAppSurvivesUnusableLock` (`single_instance_darwin_test.go:155`), driven by its own `verify.yml` step after `wails build`, extracting RT_GROUP_ICON/RT_ICON and running the existing saturation assertion against it. Owned by Story 6.2, which also picks up [[D-130]] and D-128's residue, since all three want the same CI step rather than three separate ones.

- source_spec: `spec-6-1-replace-the-placeholder-app-icon.md`
  id: D-130
  summary: verify.yml's release-proof jobs run `wails build` before `go test`, so on those two runners an icon.ico missing from git would be regenerated and satisfy the icon assertions against a file absent from the repository.
  owner: discharged
  evidence: Raised by Story 6.1's verification-gap review layer on 2026-09-19. `wails build` regenerates `build/windows/icon.ico` whenever it is absent (`packager.go:204`), and the `verify` matrix job runs that step before `go test`, so there the icon assertions can be satisfied by a file the runner just produced. The workflow's post-build drift check is scoped to `frontend/wailsjs` and `frontend/dist/.gitkeep` and does not cover `build/`. **An earlier draft overstated this as a gate-wide hole; it is not.** The `linux-adapters` job runs `go test ./...` with no `wails build` step at all -- `verify_workflow_test.go`'s `TestVerifyWorkflowLinuxJobIsAdapterVerificationOnly` actively forbids the string "wails" anywhere in that job -- so a missing `icon.ico` would fail `readICOEntries`' read there, and with it the gate. The residual risk is the narrower one that the two release-proof jobs alone would not catch it, and that the failure would surface on the job least expected to diagnose an icon problem. The fix is a two-line extension of the existing post-build drift check to `build/windows/icon.ico`. **Note on why this was deferred at all:** Story 6.1's Never clause forbids adding *an image pipeline* to `verify.yml`, not touching the file -- an earlier draft of this entry misread its own story's boundary. It is owned by Story 6.2 because it belongs with [[D-129]]'s CI step, not because 6.1 was barred from it.

- source_spec: `spec-6-2-pin-what-ships-not-the-template.md`
  id: D-131
  summary: The macOS .app's icon is unverified at the product layer, exactly as the Windows exe's was before Story 6.2.
  owner: accepted
  evidence: Raised while scoping Story 6.2 on 2026-09-19, and stated rather than left as an unstated omission. Wails derives `iconfile.icns` from `build/appicon.png` on every build (`processDarwinIcon`, `packager.go:134`), and nothing is committed under `build/darwin/` except `Info.plist` and `Info.dev.plist` -- so there is no committed `.icns` to compare a built one against, and no test opens the built `.app` bundle. Story 6.2 closed this for Windows because that is where the defect actually lived: the `.ico` is committed and `generateIcoFile` skips regeneration when it exists, which is how the scaffold placeholder shipped through v1.0.0. macOS has the opposite exposure -- the `.icns` cannot go stale, because it is regenerated unconditionally from the master every build -- so what is unverified there is narrower: that the regeneration happened and produced something resembling the master. Taking it means an env-gated darwin test in the shape of `TestDarwinBuiltAppSurvivesUnusableLock` reading `build/bin/fairdrop.app/Contents/Resources/iconfile.icns`, plus a macOS-gated `verify.yml` step. Accepted rather than routed because the macOS runner is the only place it can run and the exposure is materially smaller than the Windows one Story 6.2 closed.

- source_spec: `spec-6-2-pin-what-ships-not-the-template.md`
  id: D-132
  summary: The built exe's RT_GROUP_ICON is counted but never parsed, FIXEDFILEINFO is never read, and the exe's filename is hardcoded rather than derived from wails.json's outputfilename.
  owner: accepted
  evidence: Raised by Story 6.2's three review layers on 2026-09-20 and accepted rather than fixed in that story, which had already been through one review round. Three distinct gaps, all in `exe_resources_windows_test.go`. **RT_GROUP_ICON:** the test asserts exactly one group resource exists but never walks its GRPICONDIR entries, and the group is what Windows actually resolves to choose a size -- a group referencing one id, or ids that do not exist, satisfies the count check and every RT_ICON assertion while Windows draws the default icon. **FIXEDFILEINFO:** `versionStrings` parses only the StringFileInfo tree, so `info.json`'s `fixed.file_version` block is unasserted; a `fixed` block that stopped templating would ship a `0.0.0.0` exe with the gate green, and that block is the source of the `1.0.0.0` figure D-128's own investigation cited. **The exe path:** `builtExePath` hardcodes `fairdrop.exe` while `wails.json`'s `outputfilename` is the source of truth that `release_identity_test.go:112` already pins; renaming it makes both tests skip, and the CI step's new `--- SKIP` guard turns that into a failure rather than a silent pass, so the exposure is a confusing failure rather than a green one. All three are additive test work against a resource walk that already parses the data.
