...
- **Review round 1 (2026-09-01, patches only -- no loopback; two layers lost):** Only the edge-case layer completed. Blind Hunter and the verification-gap layer both terminated on a session rate limit mid-read, so this story has one adversarial layer plus an independent mutation pass rather than three. That is weaker coverage than every story since 1.5 and is recorded here rather than smoothed over. **Two findings were tests pinning a defect as correct**, the third story running in which that has happened. `styles.test.ts` asserted the URL field must *not* declare `min-inline-size: 0`, on the stated premise that `inline-size: 100%` makes it "sized by its grid column instead of by its content" -- which is wrong: a grid item's `min-inline-size` defaults to `auto`, resolving to the content-based minimum, and an `<input>`'s intrinsic minimum is roughly twenty characters. The percentage sets the preferred size and does nothing about the automatic minimum, so the row forced a horizontal scrollbar at the 320px floor. The `<div>` this replaced carried that declaration; converting it to an input dropped it, and the stylesheet guard could not see the loss because `inline-size: 100%` holds no pixel literal. **A repeated announcement was silent.** A polite region is re-read only when its content changes, so copying the link twice announced once; the fix swaps a keyed node, and the test asserting zero child elements was stricter than the contract, which forbids an event log rather than a child. **A copy result could speak after a focus-owned transition had already fired**, giving one moment two owners -- the single rule this story exists to keep -- so the confirmation is now refused once its own view is gone. Two smaller fixes: the readonly input lost the one-click selection the `user-select: all` div had, which is the manual fallback when the clipboard command fails; and `componentRules` in `styles.test.ts` stripped `@theme`, the dark pair and forced-colors but not reduced-motion, so universal-selector rules leaked into every component-rule assertion. **Fourteen guarantees probed by mutation, fourteen now named by a failing test.** One first-round mutation was invalid rather than surviving -- replacing the null-checked focus with `?.focus()` is the same guard by another name, and the weakening version (a non-null assertion) is caught. **Findings rejected as unreachable, with the guard that makes them so:** a regressive `bytesSent` cannot reach a selector because `progressCanFollow` refuses it; a warning code other than `beacon_warning` cannot exist because `parseWarning` admits only that one; a second warning cannot arrive because metadata is immutable per session; and a duplicate cancel is already refused by `activeCancelRef` in the controller. **KEEP:** the routing table as one pure function of `(previous, next)` returning at most one owner -- that shape is what makes "never two owners" checkable in one place instead of five views -- and the lifecycle `OutcomePanel` hoisted into a single stable slot, which is what makes "the retained node stays mounted" true of the DOM rather than of the copy.
