...
- **A measurable target, taken from measurement.** `coordinator.go` ends under 830 lines with none of the four named regions left in it; each extracted file is named for the single concern it owns and reads as a unit in one sitting. The four regions plus their three constants are 188 + ~15 lines, so 1008 becomes roughly 805. A lower target was reachable only by also moving the operation lease and the publish family, which this epic keeps with the coordination core on purpose — the count follows the scope, not the other way round.
