...
> **Superseded by review loop 3.** The numbers in this section are the loop-2
> measurements and several are now known wrong: the tolerance of 18.0 was
> replaced by a per-entry factor, and the alpha means recorded here as
> 3.19/2.47 over 3,379/3,547 samples actually measure **3.172/2.621 over
> 6758/6874**. The authoritative figures are in "Review loop 3" below, and the
> mutation table there is generated by `scripts/verify-asset-mutations.py`
> rather than written by hand. This section is kept, marked, because loop 3's
> own finding was that stale numbers survive in merged evidence files -- so
> deleting it would hide the example.
...
## Mutation table (loop 1 -- superseded by the derived table under "Review loop 3")
