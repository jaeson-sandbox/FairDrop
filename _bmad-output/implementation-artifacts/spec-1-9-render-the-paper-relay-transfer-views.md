...
- **Review round 2 (2026-08-31, patches only -- no loopback):** Raised by the human against round 1's manual-check list: a copy guarantee should not need a human. It should not, and the entry was hiding a defect rather than a gap in coverage. All three clipboard paths were already tested; what was untested was my assumption about which platform provides the API, and that is answerable from the vendored Wails source. It says: WebView2 loads `http://wails.localhost/`, and Chromium treats any `*.localhost` host as potentially trustworthy, so `navigator.clipboard` exists on Windows. WKWebView loads `wails://wails/`, a custom scheme registered with `setURLSchemeHandler:` and -- verified by grep across Wails 2.15.0's darwin frontend -- never registered as a secure scheme, so `navigator.clipboard` is undefined on macOS. **The Copy action was therefore dead on a supported platform, and `'leaves the action label alone when no clipboard exists at all'` was a test pinning that as correct** -- the same shape as the blank-window test round 1 removed. Round 1 also stated, in this spec and in a commit message, that the API "needs a secure context that a WebView2/WKWebView shell does not guarantee"; that lumps the two platforms together and is wrong about Windows. **The fix goes through Go.** Wails ships `runtime.ClipboardSetText`, which round 1 missed by checking only the generated `runtime.d.ts` instead of the Go runtime package. A fifth bound command, `CopyToClipboard`, wraps it behind the same nil-context refusal the dialogs use, bounds the text at 2048 bytes because the only legitimate payload is one capability URL, and keeps the platform's own diagnostic behind the code. The view calls it and never touches `navigator.clipboard`; a test asserts the browser API is not called even when present. `TestTheAppBindsExactlyTheContractCommands` now fixes the surface at five. **Six mutations, five caught.** The survivor is recorded rather than claimed: replacing the safe message with the platform diagnostic changes nothing observable, because `PublicErrorOf` maps `ErrTransferFailed` to fixed registry copy and discards the wrapped message -- the leak is impossible by construction, not because the test caught it.
...
- Activate Copy download link in the real window and paste the result. The command is covered on both sides, but only a real run proves the platform clipboard actually took the text.
